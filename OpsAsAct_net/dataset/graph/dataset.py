...
########################################################################
########################################################################
TASKs = ['outdegree', 'adjacent', 'connectivity', 'has-father', 'has-sister', 'grandparents', 'uncle', 'maternal-great-uncle']
len_tasks = len(TASKs)
########################################################################
...
class GraphOutDegreeDataset(GraphDatasetBase):
  ...
  def __getitem__(self, item):
    ...
    nullary_pred = np.zeros(len_tasks)
    nullary_pred[0] = 1
    return dict(
        n=graph.nr_nodes,
  ...
  def __getitem__(self, item):
    while len(self._data) == 0:
      ...
      if not self._balance_sample:
        return dict(n=family.nr_people, relations=relations, target=target, out_arity=arity, task_OneHot=nullary_pred, task_id=task_id)
